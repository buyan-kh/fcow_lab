...
const ROWS: { icon: string; label: string; chip: string; mono: boolean; detailMono: boolean; detail: DetailLine[] }[] = [
  {
    icon: "think", label: "Thinking", chip: "Ranking decision-critical evidence…", mono: false, detailMono: false,
    detail: [
      { text: "Translation is the largest unresolved source of program risk." },
      { text: "A donor-rescue study can change the next capital decision." },
    ],
  },
  {
    icon: "write", label: "Write decision record", chip: "decision-record.md", mono: true, detailMono: true,
    detail: [
      { text: "+ const gating = selectHighestValueUncertainty(program)", tone: "add" },
      { text: "+ return recommendExperiment(gating, candidates)", tone: "add" },
    ],
  },
  {
    icon: "run", label: "Rebuild and verify", chip: "rank-experiments", mono: true, detailMono: true,
    detail: [
      { text: "✓ ranked by decision value" },
      { text: "✓ falsifier criteria attached" },
    ],
  },
  {
    icon: "read", label: "Read evidence", chip: "donor-response.csv", mono: true, detailMono: false,
    detail: [
      { text: "48 samples · paired perturbation readout." },
      { text: "Signal is consistent in the reference line; donor variance is unresolved." },
    ],
  },
];

const DIFFS = [
  { file: "decision.ts", add: 13, del: 0 },
  { file: "program.ts", add: 18, del: 6 },
  { file: "evidence.json", add: 8, del: 2 },
];
...
const DIFF_LINES: Record<string, DiffLine[]> = {
  "decision.ts": [
    { text: "const uncertainty = program.gatingUncertainty;", tone: "ctx" },
    { text: "const next = cheapestExperiment(program);", tone: "del" },
    { text: "const next = rankExperiments(candidates)[0];", tone: "add" },
    { text: "}", tone: "ctx" },
  ],
  "program.ts": [
    { text: "type EvidenceItem = {", tone: "ctx" },
    { text: "  confidence: \"high\";", tone: "del" },
    { text: "  confidence: \"moderate\" | \"high\";", tone: "add" },
  ],
  "evidence.json": [
    { text: "{ \"source\": \"reference-line\" }", tone: "del" },
    { text: "{ \"source\": \"donor-derived\", \"status\": \"planned\" }", tone: "add" },
  ],
};
